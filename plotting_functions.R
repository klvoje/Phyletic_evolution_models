plot.fossil<-function (x, nse = 1, pool = FALSE, add = FALSE, modelFit = NULL, xlab=NULL, ylab=NULL,  pch = 19, lwd = 1.5, ylim = NULL, cex=NULL, ...) 
{
  if (pool) 
    x <- pool.var(x, ret.paleoTS = TRUE)
  se <- sqrt(x$vv/x$nn)
  lci <- x$mm - (nse * se)
  uci <- x$mm + (nse * se)
  xx <- x

      x$tt <- x$tt + x$start.age
      xl <- range(x$tt)

        if (!is.null(modelFit)) {
    mlab <- paste(modelFit$modelName, "expectation [95% prob. interval]")
    mc <- modelCurves(xx, w = modelFit)
    if (is.na(mc$ee[1])) 
      modelFit <- NULL
  }
  if (is.null(modelFit)) 
    yl <- c(uci, lci)
  else yl <- c(uci, lci, mc$ll, mc$uu)
  if (is.null(ylim)) 
    ylim <- range(yl)
  if (!add) 
    plot(range(x$tt), ylim = ylim, typ = "n", pch = 19, xlab=xlab, ylab=ylab,
          xlim = xl, ...)
  if (!is.null(modelFit)) {
    if (!is.null(x$start.age)) 
      mc$tt <- x$start.age - mc$tt
    polygon(c(mc$tt, rev(mc$tt)), c(mc$uu, rev(mc$ll)), col = "wheat2", 
            border = "white")
    lines(mc$tt, mc$ee, col = "tan", lwd = 2)
  }
  lines(x$tt, x$mm, lwd = lwd, ...)
  segments(x$tt, lci, x$tt, uci, lty = 1, lwd = lwd, ...)
  points(x$tt, x$mm, pch = pch, cex = 1.2, ...)
  mtext(x$label, cex = 0.7, col = "grey", font = 3)
  if (!is.null(modelFit)) 
    mtext(mlab, side = 4, cex = 0.8, col = "tan", font = 2)
}


plot.OU<-function (x, w, np = 500) 
{
  ee <- ii <- array(dim = np)
  mn <- w$modelName
  mp <- w$par
  x0 <- ifelse(w$method == "AD", x$mm[1], w$par["anc"])
  ttp <- seq(x$tt[1], x$tt[length(x$tt)], length.out = np)
  
   

      ee <- mp["theta"] * (1 - exp(-mp["alpha"] * ttp)) + mp["anc"] * exp(-mp["alpha"] * ttp)
      vv <- (mp["vstep"]/(2 * mp["alpha"])) * (1 - exp(-2 * mp["alpha"] * ttp))
  

  res <- list(tt = ttp, ee = ee, ll = ee - 1.96 * sqrt(vv), 
              uu = ee + 1.96 * sqrt(vv))
  return(res)
}


