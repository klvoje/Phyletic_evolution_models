opt.joint.URW.URW.URW<-function (y, gg, cl = list(fnscale = -1), pool = TRUE, meth = "L-BFGS-B", hess = FALSE) 
{
  if (pool) {
  ...
  small <- 1e-08
    ...
    p0rw_2 <- mle.URW(sub.paleoTS(y, ok = gg == 2))
    p0rw_3 <- mle.URW(sub.paleoTS(y, ok = gg == 3))
