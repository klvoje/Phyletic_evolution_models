opt.joint.URW.trend.Stasis<-function (y, gg, cl = list(fnscale = -1), pool = TRUE, meth = "L-BFGS-B", hess = FALSE) 
{
  if (pool) {
    y <- pool.var(y, ret.paleoTS = TRUE)
  }
  small <- 1e-08
    p0rw <- mle.URW(sub.paleoTS(y, ok = gg == 1))
    p0dt<- mle.GRW(sub.paleoTS(y, ok = gg == 2))
    p0st<- mle.Stasis(sub.paleoTS(y, ok = gg == 3))
